...
float3 normal=normalize(cross(input[2].worldPos.xyz-input[1].worldPos.xyz,input[0].worldPos.xyz-input[1].worldPos.xyz));
...
float2 maxuv=max(input[0].uv0,max(input[1].uv0,input[2].uv0));
	...
	outStream.RestartStrip();
	{
		...
		float2 viewPos = mul(input[j].worldPos.xz,float2x2(WORLDVIEW[0].x,-WORLDVIEW[0].z,WORLDVIEW[0].z,WORLDVIEW[0].x));
