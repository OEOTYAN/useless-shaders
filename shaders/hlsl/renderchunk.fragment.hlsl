...
#define ALPHA_THRESHOLD 0.5
	...
	float2 adjustmentScalar = max(1.0f / float2(length(delU), length(delV)), 1.0f);	
    float2 fractionalTexel = frac(originalUV * TEXTURE_DIMENSIONS.xy);
	float2 adjustedFractionalTexel = clamp(fractionalTexel * adjustmentScalar, 0.0f, 0.5f) + clamp(fractionalTexel * adjustmentScalar - (adjustmentScalar - 0.5f), 0.0f, 0.5f);

	float2 adjustedUV = (adjustedFractionalTexel + floor(originalUV * TEXTURE_DIMENSIONS.xy)) / TEXTURE_DIMENSIONS.xy;

    float alphacurrect = TEXTURE_0.SampleLevel(TextureSampler0,adjustedUV,0).a;
    alphacurrect=lerp(diffuse.a,alphacurrect,saturate(length(PSInput.worldPos)/8.0-1));
#else
    float alphacurrect = TEXTURE_0.SampleLevel(TextureSampler0, PSInput.uv0,0).a;
#endif
#ifndef SEASONS
if (!(PSInput.color.r + PSInput.color.g + PSInput.color.b < 2.9 &&
    ...
    float pattern0 = DP[dither0];
        if ((pattern0 / 64.0f) > alphacurrect){
            discard;
                ...
                diffuse=texture2Dat_AA(TEXTURE_0, TextureSampler0, PSInput.uv0);
            }
        }
#endif
#endif
    ...
    for(int cj=0;cj<ckk;cj++){
        float3 lchunkPos=frac((PSInput.chunkPos+chdx*ci/(float)ckk+chdy*cj/(float)ckk)/16.0)*16.0;
        float3 lcp=frac(cp+chdx*ci/(float)ckk+chdy*cj/(float)ckk);
    ...
    if (((lchunkPos.x < linea.x || lchunkPos.x > 16.0 - linea.x) &&
        ...
        ((lchunkPos.y < linea.y || lchunkPos.y > 16.0 - linea.y) &&
         (lchunkPos.z < linea.z || lchunkPos.z > 16.0 - linea.z)))
            l1+=1;
    else if (((lchunkPos.x < lineb.x ||
               lchunkPos.x > 16.0 - lineb.x) ||
              (lchunkPos.z < lineb.z ||
    ...
    diffuse.rgb=diffuse.rgb*lrrr*weightq+(lerp(diffuse.rgb, float3(0.0f, 0.0f, 1.0f), 0.2f)*weightq*l1+(diffuse.rgb / 0.4f) * (float3(1.0f, 1.0f, 1.0f) - diffuse.rgb)*weightq*l2);
    //diffuse.rgb=diffuse.rgb*lrrr*weightq+(lerp(diffuse.rgb,lerp(diffuse.rgb, float3(0.0f, 0.0f, 1.0f), 0.2f),saturate(0.0625/length(linea)))*weightq*l1+(diffuse.rgb / 0.4f) * lerp(diffuse.rgb,(float3(1.0f, 1.0f, 1.0f) - diffuse.rgb),saturate(0.0625/2/length(lineb)))*weightq*l2);
...
#ifndef S_FAR
    if (normal.y > 0.99 && abs(normal.x) + abs(normal.z) < 0.01&&PSInput.ismap < 0.5) {
...
/*
        ...
        if (abs(PSInput.chunkPos.x- 8) <linew){
            ...
            diffuse.rgb=lerp(diffuse.rgb,float3(235,90,85)/255,((40-X)/40)*0.3*(1-smoothstep(0,2*(pow(linew/0.02,7)*0.02+0.06),abs(PSInput.chunkPos.x- 8))));

        diffuse.rgb=lerp(diffuse.rgb,float3(235,90,85)/255,0.5*(1-smoothstep(0.01,2*(linew)+0.03,abs(PSInput.chunkPos.x- 8))));
*/
...
/*
    ...
    {
            #ifndef S_FAR
        if (((pattern) / 64.0f) < pow(TEXTURE_0.Sample(TextureSampler0, PSInput.uv0).a-0.03, 2.2))
        #else
    ...
    diffuse.a = 1;
#endif
}else if((abs((length(PSInput.worldPos)+8)/FAR_CHUNKS_DISTANCE-1+WATER_MIX/2.0)<WATER_MIX/2.0)){
    float kl=4.5;
        if (((pattern) / 64.0) < pow(lerp(0.02,pow(TEXTURE_0.Sample(TextureSampler0, PSInput.uv0).a-0.03,2.2*kl),((length(PSInput.worldPos)+8)/FAR_CHUNKS_DISTANCE-1+WATER_MIX)/WATER_MIX), 1/kl))
            discard;
            else
    diffuse.a = pow(lerp(diffuse.a,1,((length(PSInput.worldPos)+8)/FAR_CHUNKS_DISTANCE-1+WATER_MIX)/WATER_MIX),0.7);
...
if(PSInput.ismap > 0.5&& iswater < 0.5&&PSInput.normal.y<0.0&&min(min(PSInput.color.r,PSInput.color.g),PSInput.color.b)<0.8)
    diffuse.rgb = diffuse.rgb/lightIntensity(PSInput.normal)*lightIntensity(-PSInput.normal);
