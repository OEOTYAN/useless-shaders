...
void vert_main()
{
    ...
    float _148 = sqrt(((_140 * _140) + (_141 * _141)) + (_142 * _142));
    float _163 = ((_12_m0[20u].x - _12_m0[21u].x) + (_148 / _12_m0[21u].z)) / (_12_m0[21u].y - _12_m0[21u].x);
    float _204 = isnan(0.0f) ? _163 : (isnan(_163) ? 0.0f : max(_163, 0.0f));
    float _182;
    if (COLOR.w < 0.949999988079071044921875f)
    {
        float _177 = _148 / _12_m0[21u].w;
        float _215 = isnan(0.0f) ? _177 : (isnan(_177) ? 0.0f : max(_177, 0.0f));
        _182 = ((isnan(1.0f) ? _215 : (isnan(_215) ? 1.0f : min(_215, 1.0f))) * (1.0f - COLOR.w)) + COLOR.w;
    }
    else
    {
        _182 = COLOR.w;
    ...
    COLOR_2.w = isnan(1.0f) ? _204 : (isnan(_204) ? 1.0f : min(_204, 1.0f));
