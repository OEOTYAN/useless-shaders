...
void vert_main()
{
    ...
    float _152 = sqrt(((_144 * _144) + (_145 * _145)) + (_146 * _146));
    float _167 = ((_12_m0[4u].x - _12_m0[5u].x) + (_152 / _12_m0[5u].z)) / (_12_m0[5u].y - _12_m0[5u].x);
    float _208 = isnan(0.0f) ? _167 : (isnan(_167) ? 0.0f : max(_167, 0.0f));
    float _186;
    if (COLOR.w < 0.949999988079071044921875f)
    {
        float _181 = _152 / _12_m0[5u].w;
        float _219 = isnan(0.0f) ? _181 : (isnan(_181) ? 0.0f : max(_181, 0.0f));
        _186 = ((isnan(1.0f) ? _219 : (isnan(_219) ? 1.0f : min(_219, 1.0f))) * (1.0f - COLOR.w)) + COLOR.w;
    }
    else
    {
        _186 = COLOR.w;
    ...
    COLOR_2.w = isnan(1.0f) ? _208 : (isnan(_208) ? 1.0f : min(_208, 1.0f));
